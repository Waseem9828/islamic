...
import { Trophy, Upload, List, Info, Loader2 } from 'lucide-react';
...
export default function SubmitResultPage() {
  ...
  const handleSubmitResult = async () => {
    if (!user || !match || !selectedPosition || !screenshot) {
    ...
    try {
      ...
      await updateDoc(matchRef, { [`results.${user.uid}`]: resultData, status: 'Pending Verification' });
  ...
  return (
    <div className="container mx-auto max-w-2xl py-8">
      <Card className="border-t-4 border-yellow-500">
        <CardHeader>
          <CardTitle className="flex items-center text-2xl font-bold"><Trophy className="mr-2 h-6 w-6" /> Submit Result - {match.room}</CardTitle>
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-3 gap-4 text-center bg-muted/50 p-3 rounded-lg">
            <div><p className="text-sm text-muted-foreground">Entry Fee</p><p className="font-bold">₹{match.entry}</p></div>
            <div><p className="text-sm text-muted-foreground">Players</p><p className="font-bold">{playerCount}</p></div>
            <div><p className="text-sm text-muted-foreground">Total Pool</p><p className="font-bold">₹{totalPool}</p></div>
          </div>
          ...
          <div>
            ...
            <RadioGroup onValueChange={setSelectedPosition} className={`mt-2 grid grid-cols-2 gap-4`}>
              {prizeDistribution.map(item => (
                <Label key={item.position} htmlFor={`pos-${item.position}`} className="flex items-center space-x-2 p-4 border rounded-lg hover:bg-muted cursor-pointer has-[:checked]:bg-blue-100 has-[:checked]:border-blue-500">
          ...
          <div className="text-sm text-muted-foreground space-y-2 bg-blue-50 border border-blue-200 p-4 rounded-lg text-left">
              <h4 className="font-semibold text-blue-800 flex items-center"><Info className="mr-2 h-5 w-5"/> Verification Process:</h4>
          ...
          <Button onClick={handleSubmitResult} disabled={isSubmitting} className="w-full text-lg py-6">{isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/> Submitting...</> : 'Submit Result'}</Button>
          <Button variant="outline" className="w-full" onClick={() => router.push('/')}><List className="mr-2 h-4 w-4"/> Back to All Matches</Button>
        </CardContent>
      </Card>
    </div>
  );
