...
const ensureIsAdmin = async (context: any) => {
  ...
  const adminDoc =
    await db.collection("roles_admin").doc(context.auth.uid).get();
  if (!adminDoc.exists) {
    throw new functions.https.HttpsError(
        "permission-denied",
        "Admin-only function."
...
exports.createMatch = onCall(async (data, context) => {
  ...
  if (typeof entryFee !== "number" || entryFee < rules.minEntryFee) {
    throw new functions.https.HttpsError(
        "invalid-argument",
        `Entry fee must be at least ₹${rules.minEntryFee}.`
    );
  }
  ...
  return db.runTransaction(async (t) => {
    ...
    const deductedFromBonus = Math.min(
        remainingFee,
        walletData.bonusBalance || 0
    );
    remainingFee -= deductedFromBonus;
    const deductedFromDeposit = Math.min(
        remainingFee,
        walletData.depositBalance || 0
    );
    remainingFee -= deductedFromDeposit;
    const deductedFromWinnings = Math.min(
        remainingFee,
        walletData.winningBalance || 0
    );
    ...
    if (
      ...
      throw new functions.https.HttpsError(
          "failed-precondition",
          "Insufficient balance after detailed check."
      );
    }
...
exports.cancelMatch = onCall(async (data, context) => {
  ...
  return db.runTransaction(async (t) => {
    ...
    if (matchData.status !== "waiting" || matchData.players.length > 1) {
      throw new functions.https.HttpsError(
          "failed-precondition",
          "Cannot cancel now, other players have joined."
      );
    }
    ...
    if (creationTxQuery.empty) {
      throw new functions.https.HttpsError(
          "internal",
          "Cannot find original transaction for refund."
      );
    }
    const {
      fromDeposit = 0,
      fromWinnings = 0,
      fromBonus = 0,
    } = creationTxQuery.docs[0].data().breakdown;

    const refundAmount = matchData.entry - rules.cancellationFee;
    t.update(db.collection("wallets").doc(userId), {
       depositBalance: admin.firestore.FieldValue.increment(
          Math.min(fromDeposit, refundAmount)),
       winningBalance: admin.firestore.FieldValue.increment(
          Math.min(fromWinnings, refundAmount - fromDeposit)),
       bonusBalance: admin.firestore.FieldValue.increment(
        Math.min(fromBonus, refundAmount - fromDeposit - fromWinnings)),
    });
    ...
    return {
      status: "success",
      message: `Match cancelled. ₹${refundAmount.toFixed(
          2
      )} refunded after a ₹${rules.cancellationFee} fee.`,
    };
  });
});
...
exports.distributeWinnings = onCall(async (data, context) => {
  ...
  return db.runTransaction(async (t) => {
    ...
    if (
      ...
      throw new functions.https.HttpsError(
          "failed-precondition",
          `Match is already ${matchData.status}.`
      );
    }
    if (!matchData.players.includes(winnerId)) {
      throw new functions.https.HttpsError(
          "failed-precondition",
          "Winner was not a player."
      );
    }
...
exports.requestWithdrawal = onCall(async (data, context) => {
  ...
  return db.runTransaction(async (t) => {
    ...
    if (
      !walletDoc.exists ||
      (walletDoc.data()?.winningBalance || 0) < amount
    ) {
      throw new functions.https.HttpsError(
          "failed-precondition",
